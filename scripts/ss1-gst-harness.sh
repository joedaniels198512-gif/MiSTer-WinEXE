...
export GST_DEBUG="${GST_DEBUG:-GST_PLUGIN_LOADING:4,GST_ELEMENT_FACTORY:4,GST_PLUGIN:3,GST_STATES:4,GST_PADS:3,basesrc:4,basesink:4}"
...
timeout 25 stdbuf -oL -eL "$BOX86" "$HARNESS" "$STAGE" >>"$LOG" 2>&1
rc=$?
echo "===== after rc=$rc ====="
mem
echo "log=$LOG bytes=$(wc -c < "$LOG")"
grep -E 'HARNESS |BEFORE |AFTER |OK |FAIL |BUS |QUERY |PIPELINE |native\(wrapped\)|libgtk|Error initializing|segmentation|plugin file' "$LOG" | head -120
exit $rc
