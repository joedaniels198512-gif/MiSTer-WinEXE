...
timeout 25 stdbuf -oL -eL "$BOX86" "$HARNESS" >>"$LOG" 2>&1
rc=$?
echo "===== after rc=$rc ====="
mem
echo "log=$LOG bytes=$(wc -c < "$LOG")"
grep -E 'HARNESS |STEP |OK |FAIL |native\(wrapped\)|libgtk|Error initializing|segmentation|plugin file' "$LOG" | head -80
exit $rc
